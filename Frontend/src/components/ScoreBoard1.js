...
const SCOREBOARD1_STORAGE_KEYS = [
  "match",
  "inningNo",
  "totalRuns",
  "wicketCount",
...
const ScoreBoard = (props) => {
  ...
  const resetAllStateForNewMatch = () => {
    try {
      SCOREBOARD1_STORAGE_KEYS.forEach((k) => localStorage.removeItem(k));
  ...
  // New Result — uses `match.inning1` / `match.inning2` (final totals on `match`)
  ...
  const undoWicket = (
    ...
    const { skipRunTotalsAdjust = false } = opts;
    const ror = Math.min(4, Math.max(0, Number(runOutExtraRuns) || 0));
    ...
    const adjustStrikerUndoNbRunOutExtras = (p) => {
      ...
      const sr =
        nb > 0 ? Math.round((nr / nb) * 100 * 100) / 100 : 0;
      return { ...p, run: nr, four, strikeRate: sr };
    };
    ...
    if (dismissed.onStrike) {
      ...
      if (!isNoBallParam) {
        dismissed.ball = Math.max(0, dismissed.ball - 1);
      }
    ...
    if (batter1.name === undefined || batter1.onStrike) {
      const batter1NameElement = document.getElementById("batter1Name");
      batter1NameElement.value = dismissed.name;
      ...
      const batter2NameElement = document.getElementById("batter2Name");
      batter2NameElement.value = dismissed.name;
    ...
    if (currentRunStack.length > 0) {
      ...
      if (typeof last === "number") {
        ...
        if (last === "W") {
          ...
          undoWicket(true, wr, { skipRunTotalsAdjust: true });
        } else {
  ...
  const handleWicket = (isRunOut, playerId, runOutCompletedRunsParam = 0) => {
    ...
    const nRaw = Math.min(
      4,
      Math.max(0, Number(runOutCompletedRunsParam) || 0),
    );
    ...
    const legalWicketToken =
      isRunOut && runsAddLegal > 0 ? `W+${runsAddLegal}` : "W";
    ...
    } else if (runsAddNbModal > 0) {
      if (strikerIsBatter1) {
        b1ForOut = applyNbRunOutRunsOnly(batter1, runsAddNbModal);
      } else {
        b2ForOut = applyNbRunOutRunsOnly(batter2, runsAddNbModal);
      }
    }
    ...
    const runsForTotals = runsAddLegal || runsAddNbModal;
    if (runsForTotals > 0) {
    ...
    if (isCompositeNbRunOut) {
      ...
      if (ballCount === 5) {
        setTotalOvers(overCount + 1);
        overCompleted(runsByOver, [...stackPrefix, tok]);
        setWicketCount(wicketCount + 1);
        disableAllScoreButtons();
      } else {
      ...
      if (isNoBall) {
        ...
        overCompleted(runsByOver + runsAddLegal, arr);
        setWicketCount(wicketCount + 1);
        disableAllScoreButtons();
      }
    } else {
  ...
  const remainingRunsContent3 = (
    <>
      ...
      {hasMatchEnded ? (
        <div>
          Click &quot;Save&quot; on the button above to save both innings to the
          database.
        </div>
      ) : (
        <div>Open the Score Card tab to view the full scorecard.</div>
      )}
    </>
  );
